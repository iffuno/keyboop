...
enum Warm {
    ...
    static func prime() {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // Таблица опечаток грузится здесь же: файл небольшой, но на пути нажатия ему не место.
            // Загружаем ВСЕГДА, а не только при включённой настройке: личный словарь должен копиться
            // с первого дня, иначе включённая через месяц функция начнёт править привычные человеку
            // слова (см. TypoFix).
            TypoFix.shared.load()
        }
    }
