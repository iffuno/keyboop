...
final class AudioRecorder: NSObject {
    ...
    /// Отпустить микрофон СРАЗУ, не оставляя тёплым.
    ///
    /// ⚠️ Заведено по отзыву #114 (09.08.2026): «после отжатия правого cmd микрофон не освобождается
    /// и звук у других программ искажается». Про оранжевый индикатор мы знали и написали об этом
    /// ниже, а вот что удержание устройства портит звук ЧУЖИМ программам, не предполагали.
    ///
    /// В логе того отзыва видно и повод: четыре подхода подряд по 0.06–0.24 секунды, все отброшены
    /// как «слишком коротко». Человек тычет в модификатор, а не удерживает его. Тёплое окно нужно,
    /// чтобы ускорить ПОВТОРНУЮ диктовку, а несколько отброшенных подходов означают ровно обратное:
    /// сейчас человек не диктует, и держать за него микрофон полминуты не за чем.
    func releaseNow() {
        releaseEngine()
        kbLog("voice rec: запись отброшена — микрофон отпущен сразу, без тёплого окна")
    }
