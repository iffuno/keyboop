...
enum Changelog {
    /// `announce` / `announceItems` / `announceEnd` видит ТОЛЬКО Telegram: в окне «Что нового» они
    /// не показываются. `ru` / `en` остаются точным и сухим пользовательским журналом, а Telegram
    /// получает отдельные живые пункты с голосом автора. Порядок в посте: announce → announceItems →
    /// announceEnd. Если announceItems нет, старые записи по-прежнему используют `ru`.
    struct Release {
        let version: String
        let ru: [String]
        let en: [String]
    ...
    static let releases: [Release] = [
        ...
        Release(version: "0.4.7",
            ru: [
                ...
                "Конвертация слов через дефис стала точнее. «xnj-nj» превращается в «что-то», «ult-nj» — в «где-то», а «bp-pf» — в «из-за». Командные флаги, диапазоны, идентификаторы, висячие и двойные дефисы не трогаются.",
                "Точка по двойному пробелу надёжнее переживает автоматическую конверсию. В основном сценарии Keyboop сохраняет «. » независимо от того, успела ли macOS поставить точку сама. Более редкое совпадение второго пробела с самой синтетической заменой пока остаётся известным ограничением.",
            ],
            en: [
                "A valid word with one trailing punctuation mark is no longer converted only in part. For example, “got it.” stays entirely English instead of becoming mixed-language text. This narrow protection applies to automatic correction; manual conversion still follows an explicit user command.",
                "Hyphenated word conversion is more accurate. “xnj-nj” becomes “что-то”, “ult-nj” becomes “где-то”, and “bp-pf” becomes “из-за”. Command-line flags, ranges, identifiers, dangling hyphens, and doubled hyphens are left alone.",
                "The double-space period survives automatic conversion more reliably. In the main timing case, Keyboop preserves “. ” whether or not macOS inserted the period first. A rarer overlap between the second space and the synthetic replacement remains a known limitation.",
            ],
            beta: true,
            announce: """
                Вселенная меня услышала. Судя по всему, она ещё и читает релиз-ноты: сейчас \
                устраиваюсь на работу. На Keyboop останется меньше времени, зато режим «спорить с \
                одной точкой до утра» будет включаться реже.

                Я уже пользуюсь Keyboop каждый день и ругаюсь всё реже. Это не финальные титры: \
                фидбэк читаю, ошибки чиню, новое добавляю. Просто теперь спокойнее.
                """,
            announceItems: [
                "«got it.» больше не становится русско-английским дипломатическим кризисом: точка оставляет правильное слово в покое.",
                "Слова через дефис теперь путешествуют вместе: «xnj-nj» становится «что-то», а технические конструкции не трогаем.",
                "Точка по двойному пробелу надёжнее переживает автоконверсию. У macOS был свой авторский замысел.",
            ],
            announceEnd: """
                Спасибо за донаты. Они финансируют исследования характера точки: \
                keyboop.com/tips или boosty.to/iffun

                Для самостоятельной подачи Keyboop в Homebrew Cask нужно немного звёзд. GitHub \
                раздаёт их бесплатно, Homebrew почему-то считает всерьёз: github.com/iffuno/keyboop
                """),
        ...
        // выравнивание значка в строке меню. Бета по протоколу: правки лежат в горячем пути набора
        // и в диктовке, а такие классы обязаны сначала обкататься на тех, кто сам вызвался.
        Release(version: "0.4.1",
            ru: [
                "Первая диктовка после запуска больше не заставляет ждать. Модель распознавания умудрялась загрузиться дважды подряд, если продиктовать сразу после старта, и вместо трети секунды выходило под сорок. Теперь загрузка одна, а вторая просьба спокойно дожидается первой.",
                "Исправление опечаток перестало портить правильные слова. Оно меняло последнюю букву, а в русском это окончание, поэтому «раскладки» превращались в «раскладку», а «фигма» в «фирму». Последняя буква теперь неприкосновенна, соседство клавиш считается по их настоящему положению на клавиатуре, а «фигма», «джира», «слак» и другие заимствования добавлены в словарь целиком.",
                "Диктовка выучила USDT, VPN, Seedance, Seedream, Kling и Higgsfield. Так распознавание слышит эти слова: «юс ди ти», «впен», «хигсвилд». Список открыт, свои слова можно дописать в настройках диктовки.",
                "Значок в строке меню встал ровно. Он сидел на полтора пункта выше соседних значков: рисованной картинке система не знает базовой линии, в отличие от системных символов.",
            ],
            en: [
                "The first dictation after launch no longer keeps you waiting. The recognition model managed to load twice in a row if you spoke right after start-up, so a third of a second turned into nearly forty. Now it loads once and the second request simply waits for the first.",
                "Typo correction stopped breaking words that were already right. It used to replace the last letter, which in Russian is the ending, so «раскладки» became «раскладку» and «фигма» became «фирма». The last letter is now left alone, key neighbours are computed from their real position on the keyboard, and borrowed names like Figma, Jira and Slack are in the dictionary as whole words.",
                "Dictation learned USDT, VPN, Seedance, Seedream, Kling and Higgsfield. These are the words recognition writes the way it hears them. The list is open: add your own in the dictation settings.",
                "The menu bar icon is aligned. It sat one and a half points above its neighbours, because a drawn image carries no baseline for the system to use, unlike a system symbol.",
            ],
            announce: """
                Так увлёкся разработкой, что сбил режим: ложусь под утро, встаю после обеда. \
                Здоровью это нравится примерно никак, так что совет простой: следите за собой \
                и не делайте, как я.

                Про поломки пишут всё меньше, всё больше просят нового. Читаю все.

                Этот выпуск целиком про починку. Две вещи, которые раздражали молча: долгая \
                первая диктовка и автозамена, которая правила то, что править не просили.

                Из планов: облачная обработка текста, не ломая офлайн, и переводчик получше.
                """,
            announceEnd: """
                Бета сама не приедет: включите «Ставить бета-версии» и нажмите «Проверить обновления».

                Спасибо тем, кто закинул на чай. Приложение бесплатное и платным не станет, \
                это чисто по желанию: keyboop.com/tips

                Хорошего настроения и нормального сна.
                """),
        Release(version: "0.4",
            ru: [
        ...
        Release(version: "0.3.19",
            ru: [
                "Исправление опечаток больше не замолкает на повторах. Слово не из словаря, набранное дважды, программа считала вашим и переставала его трогать, а привычная опечатка это как раз то, что набирают не один раз. Теперь как ваши запоминаются только слова, для которых у программы нет готовой правки, а список разбирается при первом запуске.",
            ],
            en: [
                "Typo correction no longer goes quiet on repeats. A word missing from the dictionary that you typed twice counted as yours and stopped being touched, while a habitual typo is exactly the thing you type more than once. Only words the app has no correction for are remembered as yours now, and entries that got onto that list by mistake are cleared on first launch.",
            ],
            announce: """
                Выпускаю патч к сегодняшнему релизу, пока всё свежо в голове.
                """,
            announceEnd: """
                Если Keyboop давно не обновлялся у вас сам, перезапустите его один раз. В прежних \
                версиях скачанное обновление ждало, пока вы закроете программу, а её в панели меню \
                никто не закрывает. После перезапуска всё поедет само.

                И раз уж я тут, давняя просьба. Homebrew пускает автора паковать себя самого только \
                с 225 звёзд на GitHub, ровно чтобы авторы себя не паковали. У нас 109, так что если \
                не жалко одного клика: github.com/iffuno/keyboop
                """),
