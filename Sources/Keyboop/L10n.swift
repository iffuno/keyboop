...
enum L10n {
    ...
    private static let strings: [String: [Lang: String]] = [
        ...
        "voice.streamingSub":[.ru: "Печатает, пока ты ещё говоришь. Иногда читает мысли, иногда фантазирует — может икнуть. Нужна отдельная модель (докачается).",
                              .en: "Types while you're still talking. Sometimes reads your mind, sometimes improvises — may hiccup. Needs a separate model (will download)."],
        ...
        "hist.lock.toggleSub":[.ru: "Спрошу пароль при открытии окна истории. Защита от любопытных глаз за твоим Mac; на диске история и так зашифрована.",
                               .en: "Asks for a password when opening the history window. Keeps curious eyes at your Mac out; on disk the history is encrypted anyway."],
