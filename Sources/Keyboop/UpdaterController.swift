...
final class UpdaterController: NSObject, SPUUpdaterDelegate {
    ...
    func checkNow() {
        // ⚠️ ЗАПИСЬ О САМОМ НАЖАТИИ, ДО ВСЯКОЙ ЛОГИКИ (13.09.2026, отзыв #268). Человек пишет:
        // «нажимаешь обновить, ничего не происходит; заходишь в раздел Обновления, жмёшь проверить
        // сейчас, тоже ничего». В его диагностике за всё окно лога НЕТ НИ ОДНОЙ строки про
        // обновление, и это значит ровно одно: мы не знаем, дошло ли нажатие до нас вообще. Разбор
        // упирается в отсутствие следа, а не в загадку. Одна строка здесь стоит ноль и превращает
        // «непонятно» в «видно».
        kbLog("updater: нажата проверка вручную (версия \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "?"), канал \(AppSettings.shared.betaChannel ? "beta" : "стабильный"))")
